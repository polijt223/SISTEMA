...
export default {
    ...
    add: async(req,res,next) =>{
        try{
            const reg = await models.VentaModel.create(req.body);
            //Actualizar stock 
    ...
    query: async(req,res,next) =>{
        try{
            ...
            const reg = await models.VentaModel.findOne({_id: req.query._id})
    ...
    list: async(req,res,next) =>{
        try{
            ...
            const reg = await models.VentaModel.find({$or:[{'num_comprobante': new RegExp(valor,'i')},{'serie_comprobante': new RegExp(valor,'i')}]})
            .populate('usuario',{nombre:1})
            .populate('persona',{nombre:1})
            .sort({'createAt':-1});
            res.status(200).json(reg);
    ...
    activate: async(req,res,next) =>{
        try{
            const reg = await models.VentaModel.findByIdAndUpdate({_id: req.body._id},{estado:1});
            //Actualizar stock 
    ...
    desactivate: async(req,res,next) =>{
        try{
            const reg = await models.VentaModel.findByIdAndUpdate({_id: req.body._id},{estado:0});
            //Actualizar stock 
            let detalle = reg.detalles;
            detalle.map( (x) => {
    ...
    grafico12Meses: async(req,res,next) => {
        try {
            const reg = await models.VentaModel.aggregate(
                [
                    {
                        $group:{
                            _id:{
    ...
    consultaFechas: async(req,res,next) =>{
        try{
            ...
            const reg = await models.VentaModel.find({"createAt":{"$gte":start, "$lt":end}})
            .populate('usuario',{nombre:1})
            .populate('persona',{nombre:1})
            .sort({'createAt':-1});
            res.status(200).json(reg);
